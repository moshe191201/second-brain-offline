# CLAUDE.md — Example Vault Schema

> Replication runbook: `instructions.md` · Operating skills: `.claude/skills/vault-*` · CLI: `scripts/vault.py`
> Framework-owned file. Edit only inside the USER ZONE at the bottom — everything else is
> replaced wholesale by `vault upgrade`.
...
`tests/` — eval fixtures and gold answers. **Never register as a qmd collection.**
...
3. If the vault does **not** cover it, say so explicitly. Do not fill the gap from
   ...
   the T2 negative-control tests in `tests/VAULT_TESTS.md` verify.)
...
- Never edit `raw/`. If `vault check` fails and you cannot fix it within the skill's
...
2. `python3 scripts/vault.py ingest raw/<clipping>.md` — stubs the summary, registry row, log entry
3. Fill `wiki/sources/<slug>.md`, then create concept notes with
   `python3 scripts/vault.py new-note <slug> --source raw/<clipping>.md`
4. Update `index/_map-of-content.md` and `index/key-takeaways.md`
5. `python3 scripts/vault.py check` — must exit 0
...
1. `python3 scripts/vault.py check` — fail-closed; fix every finding before proceeding
...
Everything between the markers survives `vault upgrade`. Put domain notes, extra
conventions, and local overrides here — nowhere else in this file.

<!-- USER ZONE START -->

**Domain:** LLM finetuning and inference — LoRA/QLoRA, SFT, RLHF/GRPO, quantization,
KV cache and serving, multimodal (vision/TTS) finetuning, plus frontier-model clippings.

**This vault is living documentation.** It is the one vault that lives inside the framework
repo, and CI asserts it matches the current payload. Framework-owned files here are
artifacts — edit them in `src/second_brain_vault_framework/payload/` and run
`vault upgrade example_vault`. This USER ZONE and the content layers (`raw/`, `wiki/`,
`index/`) are the only things edited in place.

**Design history:** `docs/superpowers/specs/` (the eval design is
`2026-06-11-vault-improvements-and-eval-design.md`) · plans in `docs/superpowers/plans/`.
