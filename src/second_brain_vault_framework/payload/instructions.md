...
├── scripts/        TOOLING — vault.py: deterministic CLI + lint (CLAUDE.md Lint workflow);
│                              check_vault_answer.py: grades a recorded answer vs. tests/.
├── tests/          TESTS   — VAULT_TESTS.md manual checklist. NEVER a qmd collection.
...
└── .qmd/           ENGINE B — hybrid BM25 + vector search over raw/, wiki/, index/ (NOT tests/).
...
The framework (`CLAUDE.md`, the `vault-*` skills, `scripts/vault.py`, `scripts/check_vault_answer.py`,
`instructions.md`, `tests/VAULT_TESTS.md`) is packaged as **`second-brain-vault-framework`** and published
...
|------|-------|--------------------|
| **Framework-owned** | `CLAUDE.md`, `instructions.md`, `.claude/skills/vault-*/SKILL.md`, `scripts/vault.py`, `scripts/check_vault_answer.py`, `tests/VAULT_TESTS.md` | Overwritten wholesale from the package payload. Read-only by convention. |
...
- **`scripts/vault.py check`** — pure-stdlib deterministic lint (broken wikilinks, orphans,
  unreferenced clippings, missing `sources:`, log coverage, MOC reachability, duplicate
  stems); exits non-zero on any finding.
- **`tests/VAULT_TESTS.md`** — manual test checklist (T0 lint · T1 known-answer · T2
  negative controls · T3 cross-source synthesis · T4 engine smoke · T5 incremental ingest).
  `tests/` is deliberately **never** registered as a qmd collection so gold answers cannot
...
tar czf vault-control.tgz -C <source-vault> CLAUDE.md scripts/vault.py scripts/check_vault_answer.py tests/VAULT_TESTS.md
```
Windows (PowerShell):
```powershell
# A1. Node.js + Obsidian installers for the target platform → grab from vendor sites.
...
7. `vault-control.tgz` (CLAUDE.md · scripts/vault.py · scripts/check_vault_answer.py · tests/VAULT_TESTS.md)
...
mkdir -p "<Vault>"/{raw,wiki,index,scripts,tests}
...
# tests/VAULT_TESTS.md is now a TEMPLATE: its gold facts (T1–T3) describe the OLD corpus.
# Rewrite them for the NEW corpus before testing — the structure carries over, answers do not.
```
Windows (PowerShell):
```powershell
# C1. Vault skeleton:
'raw','wiki','index','scripts','tests' | ForEach-Object { New-Item -ItemType Directory -Force "<Vault>\$_" | Out-Null }
...
tar xzf vault-control.tgz -C "<Vault>\"
# tests/VAULT_TESTS.md is now a TEMPLATE describing the OLD corpus — rewrite T1–T3 for the new one.
```
...
> and `index/log.md`; register the `raw/`, `wiki/`, `index/` qmd collections (NOT `tests/`),
> then `qmd update && qmd embed`; build the knowledge graph with `/graphify .`. Obey the
> safety rules and the vault-grounding rule. Finally run `python3 scripts/vault.py check`
...
qmd collection add ./wiki  --name concepts
qmd collection add ./index --name indices
# NEVER: qmd collection add ./tests — gold answers must not enter retrieval.
qmd update && qmd embed
...
qmd collection add ./wiki  --name concepts
qmd collection add ./index --name indices
# NEVER: qmd collection add ./tests
qmd update; qmd embed
...
macOS / Linux (bash):
```bash
python3 scripts/vault.py check                  # deterministic structure check — must exit 0
qmd collection list                            # confirm tests/ is NOT among the collections
...
```powershell
py scripts\vault.py check                        # must exit 0
qmd collection list                             # confirm tests/ is NOT a collection
qmd status
...
For a richer behavioral check, run the `tests/VAULT_TESTS.md` checklist (T0–T4): T1
...
- `python3 scripts/vault.py check` exits 0; `tests/` is absent from `qmd collection list`;
...
6. `python3 scripts/vault.py check` and fix any findings until it exits 0.

(Steps 4–6 are the `CLAUDE.md` **Ingest** workflow — the single source of truth for the loop.)

On Windows (PowerShell): `qmd update; qmd embed` (step 2) and `py scripts\vault.py check`
...
wiki/sources/ summaries, scripts/vault.py, tests/VAULT_TESTS.md). Versions:
